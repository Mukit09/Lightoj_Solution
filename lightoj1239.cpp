    ...
    scanf("%d",&t);
   	while(t--)
   	{
        ...
        if(n<3)
        {
            ans=2.0*pi*d;
            if(n==2)
            ans+=2.0*sqrt((double)dist((double)stru[0].x,(double)stru[0].y,(double)stru[1].x,(double)stru[1].y));
            goto prnt;
        }
        sort(stru,stru+n,cmp1);
        pvt=stru[0];
        sort(stru,stru+n,cmp);
        C[0]=stru[0],C[1]=stru[1];
        j=2;
        rep(i,2,n)
        {
        ...
        C[j]=C[0];
        rep(i,0,j)
        ans+=sqrt((double)dist((double)C[i].x,(double)C[i].y,(double)C[i+1].x,(double)C[i+1].y));
        ans+=2.0*pi*d;
        prnt:
        printf("Case %d: %.10lf\n",cs++,ans+eps);
    }
